...
module.exports = {
    ...
    async update(req, res) {
        ...
        let datas = {
            ...
            "pendente": req.body.pendente || false,
            "pendenciaDescricao": req.body.pendenciaDescricao || null
        }
...
INNER JOIN clientes ON atendimentos.id_cliente = clientes.id 
INNER JOIN tipos_atendimentos ON atendimentos.id_tipo_atendimento = tipos_atendimentos.id
WHERE usuarios.id = ${id} AND tarefas.data = CURRENT_DATE
ORDER BY tarefas.data DESC;
`);
            res.json(response[0]);
        } catch (error) {
            console.log(error);
